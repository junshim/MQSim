...
#define ERS_CANCEL_ENABLE             (false) //true 
	...
	#define ERS_IDEAL_SUS_ENABLE      (false)//false //ERASE가 suspend되더라도 패널티가 없음. 
	#define ERS_SUS_FAST12            (false)//false
	#define ADAPTIVE_RPS_SCH_ENABLE   (false)

#define PGM_SUS_ENABLE        (false) //false

#define MAX_SUBBLOCK_ERS_CNT 4//if 2-> can erase total 3 sub-blocks from 1 block. 7 is max
#define INVALID_GRANU 10 
	#define THRESHOLD_INVALID 8

#define REMOVE_LATENCY 0 //만약 이게 1이라면 start_command_execution에서 subblock erase의 경우 erase latency를 dieBKE->time관련에 더하지 않으므로 service_read_transaction에서 추가적인 subblk erase때문에 read가 서비스 못받는경우는 없음
#define REMOVE_LATENCY_SEND 0 //send_command_to_chip(), Execute_simulator_event()
#define REMOVE_LATENCY_TMP 0 //PRIMARY_ERS관련. 만일 preemptible와 PE SUSPEND/RESUME으로 인한 free block 감소를 볼 시, 0으로 세팅
