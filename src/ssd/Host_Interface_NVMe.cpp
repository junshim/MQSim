...
namespace SSD_Components
{
	...
	{
		//std::cout << "Handle_new_arrived_request: " << request->ID << ",response time: " << Simulator->Time() - request->STAT_InitiationTime << std::endl;
		sarsa_unit->Update_Interval(request->STAT_InitiationTime, request->ID);
	...
	inline void Input_Stream_Manager_NVMe::Handle_serviced_request(User_Request* request)
	{
		...
		//std::cout << "Handle_serviced_request: " << request->ID << ",delay time: " << Simulator->Time() - request->STAT_InitiationTime << std::endl;

		sarsa_unit->Broadcast_Reward(Simulator->Time() - request->STAT_InitiationTime, request->ID);
