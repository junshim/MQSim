...
FILE* free_log_file = fopen("free_block_log.csv", "w");
#define FREE_BLOCK_LOG 1
extern bool finished_preconditioning;

extern Sarsa* sarsa_unit;
...
namespace SSD_Components
{
			...
			dynamic_wearleveling_enabled, static_wearleveling_enabled, static_wearleveling_threshold, seed)
	{
		...
		free_block_out.open("free_block_num.csv");
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address, bool is_urgent)
	{
		free_block_out << plane_address.ChannelID << ", " << plane_address.ChipID << ", " << free_block_pool_size << "\n";
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			if (pbke->Ongoing_erase_operations.size() > 0 || sarsa_unit->Is_Wait_Reward(plane_address)) {
				return;
			}
			
			size_t gc_block_cnt = 1;

			std::vector<flash_block_ID_type> gc_candidate_block_ids;
			...
			{
				//unsigned int action_cnt =  : 3;
				unsigned int action_cnt = (is_urgent) ? 1 : sarsa_unit->Get_Action(plane_address);
				for (unsigned int num = 0; num < action_cnt; num++) {

					unsigned int min_invalid_page_count = 0;
					bool foundGC_candidate = false;
					for (flash_block_ID_type block_id = 1; block_id < block_no_per_plane; block_id++) {
						if (pbke->Blocks[block_id].Invalid_page_count > min_invalid_page_count && check_block_id(block_id, gc_candidate_block_ids)
							&& pbke->Blocks[block_id].Current_page_write_index > 0
							&& is_safe_gc_wl_candidate(pbke, block_id)) {
							min_invalid_page_count = pbke->Blocks[block_id].Invalid_page_count;
							gc_candidate_block_id = block_id;
							foundGC_candidate = true;
						}
					}
					if (foundGC_candidate)
						gc_candidate_block_ids.push_back(gc_candidate_block_id);
					else
						break;
				}
				gc_block_cnt = gc_candidate_block_ids.size();
				break;
			}
			...
			for (size_t i = 0; i < gc_block_cnt; i++) {
				if (block_selection_policy == SSD_Components::GC_Block_Selection_Policy_Type::GREEDY)
					gc_candidate_block_id = gc_candidate_block_ids[i];
	...
	bool GC_and_WL_Unit_Page_Level::check_block_id(flash_block_ID_type block_id, std::vector< flash_block_ID_type>& block_ids) {
		for (int i = 0; i < block_ids.size(); i++)
			if (block_id == block_ids[i])
				return false;
		return true;
	}
