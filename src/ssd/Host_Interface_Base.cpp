...
sim_time_type user_write_latency;
sim_time_type avg_user_write_latency;
FILE* user_write_latency_log_file = fopen("user_write_latency_log_file.txt", "w");
...
namespace SSD_Components
{
	...
	void Input_Stream_Manager_Base::Update_transaction_statistics(NVM_Transaction* transaction)
	{
		...
		case Transaction_Type::WRITE:
			...
			//[bkhan]
			user_write_latency = (transaction->STAT_execution_time + transaction->STAT_transfer_time + (Simulator->Time() - transaction->Issue_time) - transaction->STAT_execution_time - transaction->STAT_transfer_time) / SIM_TIME_TO_MICROSECONDS_COEFF;

			avg_user_write_latency = (input_streams[0]->STAT_sum_of_write_transactions_execution_time + input_streams[0]->STAT_sum_of_write_transactions_transfer_time + input_streams[0]->STAT_sum_of_write_transactions_waiting_time) / input_streams[0]->STAT_number_of_write_transactions / SIM_TIME_TO_MICROSECONDS_COEFF;

			fprintf(user_write_latency_log_file, "%lu, %lu,\n", user_write_latency, avg_user_write_latency);
			break;
		default:
			break;
		}
	...
	uint32_t Input_Stream_Manager_Base::Get_average_write_transaction_turnaround_time(stream_id_type stream_id)//in microseconds
	{
		if (input_streams[stream_id]->STAT_number_of_write_transactions == 0)
			return 0;
		/*avg_user_write_latency = (uint32_t)((input_streams[stream_id]->STAT_sum_of_write_transactions_execution_time + input_streams[stream_id]->STAT_sum_of_write_transactions_transfer_time + input_streams[stream_id]->STAT_sum_of_write_transactions_waiting_time)
			/ input_streams[stream_id]->STAT_number_of_write_transactions / SIM_TIME_TO_MICROSECONDS_COEFF);
		return avg_user_write_latency;*/
		return (uint32_t)((input_streams[stream_id]->STAT_sum_of_write_transactions_execution_time + input_streams[stream_id]->STAT_sum_of_write_transactions_transfer_time + input_streams[stream_id]->STAT_sum_of_write_transactions_waiting_time) / input_streams[stream_id]->STAT_number_of_write_transactions / SIM_TIME_TO_MICROSECONDS_COEFF);
	}
