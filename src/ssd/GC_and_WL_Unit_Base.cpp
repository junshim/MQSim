...
namespace SSD_Components
{
		...
		dynamic_wearleveling_enabled(dynamic_wearleveling_enabled), static_wearleveling_enabled(static_wearleveling_enabled), static_wearleveling_threshold(static_wearleveling_threshold)
	{
		...
		is_GC_on_Going = new bool** [channel_count];
		for (unsigned int nCHIter = 0; nCHIter < channel_count; nCHIter++) {
			is_GC_on_Going[nCHIter] = new bool* [chip_no_per_channel];
			for (unsigned int nChipIter = 0; nChipIter < chip_no_per_channel; nChipIter++) {
				is_GC_on_Going[nCHIter][nChipIter] = new bool[plane_no_per_die];
				for (unsigned int nPlaneIter = 0; nPlaneIter < plane_no_per_die; nPlaneIter++) {
					...
					is_GC_on_Going[nCHIter][nChipIter][nPlaneIter] = false;
				}
			}
		}
	}
	...
	GC_and_WL_Unit_Base::~GC_and_WL_Unit_Base(){
		for (unsigned int nCHIter = 0; nCHIter < channel_count; nCHIter++) {
			for (unsigned int nChipIter = 0; nChipIter < chip_no_per_channel; nChipIter++) {
				delete[] is_GC_on_Going[nCHIter][nChipIter];
			}
			delete[] is_GC_on_Going[nCHIter];
		}
		delete[] is_GC_on_Going;
	}
...
//				std::cout << "GC Trigger : " << transaction->Address.ChannelID << " : " << transaction->Address.ChipID << " : " << transaction->Address.PlaneID << " : " << pbke->Get_free_block_pool_size() << std::endl;
				...
				_my_instance->Check_gc_required(pbke->Get_free_block_pool_size(), transaction->Address, true);
			}
			break;
		}
	}
