...
#if REMOVE_LATENCY_SEND
	inline ChipStatus NVM_PHY_ONFI_NVDDR2::GetChipSubStatus(NVM::FlashMemory::Flash_Chip* chip)
	{
		return bookKeepingTable[chip->ChannelID][chip->ChipID].subStatus;
	}
#endif
...
//				if ( transaction_list.front()->Address.ChannelID == 3 && transaction_list.front()->Address.ChipID == 3 )
		...
		case Transaction_Type::ERASE: //한번에 최대 plane갯수까지만 CMD_ERASE_BLOCK_MULTIPLANE
			...
			for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
				it != transaction_list.end(); it++) {
#if REMOVE_LATENCY_SEND
				//if ((erase_tr_cnt != 0) && (subblock_erase_tr_cnt!=0)) { //1229
				if ((erase_tr_cnt != 0) && subblock_erase_tr_cnt == erase_tr_cnt ) { //1229
					// do nothing //(*it)->STAT_transfer_time += target_channel->EraseCommandTime[transaction_list.size()];
				}
				else {
			...
			if (chipBKE->OngoingDieCMDTransfers.size() == 0)
			{
				...
				if ((erase_tr_cnt != 0) && subblock_erase_tr_cnt == erase_tr_cnt) { //1229
					...
					chipBKE->Last_transfer_finish_time = Simulator->Time();
					Simulator->Register_sim_event(Simulator->Time(),
						this, dieBKE, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 1, NOT_GC_WL);
					//std::cout << "1. [subers] register event as subblock erase" << std::endl;
				}
#if REMOVE_LATENCY_TMP
				else if ((erase_tr_cnt != 0) && subblock_erase_tr_cnt != erase_tr_cnt) {
					targetChip->StartCMDXfer();
					chipBKE->Status = ChipStatus::CMD_IN;
					chipBKE->Last_transfer_finish_time = Simulator->Time();
					Simulator->Register_sim_event(Simulator->Time(),
						this, dieBKE, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 2, NOT_GC_WL);
				}
#endif
				else {
					targetChip->StartCMDXfer();
					...
					Simulator->Register_sim_event(Simulator->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()], 
						this, dieBKE, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 0, NOT_GC_WL);
				}
#else
				targetChip->StartCMDXfer();
				chipBKE->Status = ChipStatus::CMD_IN;
			...
			if ((erase_tr_cnt != 0) && subblock_erase_tr_cnt == erase_tr_cnt) { //1229
				...
				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID);
				//dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time; ////1
			}
#else
			dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID);
#endif
...
#if REMOVE_LATENCY_SEND
				if (ev->is_subERS == 1) {
					//std::cout << "2.[subers] ev->is_subERS=true (Execute_simulator_event)" << std::endl; //checked
					chipBKE->subStatus = ChipStatus::ERASING_SUBBLOCK;
					bookKeepingTable[channel_id][targetChip->ChipID].subStatus =  ChipStatus::ERASING_SUBBLOCK;
					/*
...
#if REMOVE_LATENCY_TMP
				else if (ev->is_subERS == 2) {
					chipBKE->subStatus = ChipStatus::PRIMARY_ERS; //erase를 먼저 수행
					bookKeepingTable[channel_id][targetChip->ChipID].subStatus = ChipStatus::PRIMARY_ERS;
				}
#endif
				else if(!ev->is_subERS){ 
					//std::cout << "ev->is_subERS=false (Execute_simulator_event)" << std::endl;
					//chipBKE->subStatus = ChipStatus::IDLE;
					//bookKeepingTable[channel_id][targetChip->ChipID].subStatus = ChipStatus::IDLE;
				}
#endif
				targetChannel->SetStatus(BusChannelStatus::IDLE, targetChip);
			}
...
//			if ( nCH_ID == 3 && nCHIP_ID == 3 )
		...
		if (WaitingCopybackWrites[channel_id].size() > 0)
		{
			...
			Simulator->Register_sim_event(Simulator->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()],
				this, waitingBKE, (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL);
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				Simulator->Register_sim_event(Simulator->Time() + _my_instance->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()],
					_my_instance, dieBKE, (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL);
				chipBKE->OngoingDieCMDTransfers.push(dieBKE);
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		Simulator->Register_sim_event(Simulator->Time() + NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID]),
			this, dieBKE, (int)NVDDR2_SimEventType::READ_DATA_TRANSFERRED, 0, NOT_GC_WL);
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		case Transaction_Type::READ:
			...
			Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
				this, bookKeepingEntry, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL);
			break;
		case Transaction_Type::WRITE:
			if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->RelatedRead == NULL) {
				...
				Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
					this, bookKeepingEntry, (int)NVDDR2_SimEventType::PROGRAM_CMD_ADDR_DATA_TRANSFERRED, 0, NOT_GC_WL);
			}
			else
			{
				chip->StartCMDXfer();
				...
				Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime, this,
					bookKeepingEntry, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL);
			}
			break;
		case Transaction_Type::ERASE:
			chip->StartCMDXfer();
			bookKeepingTable[chip->ChannelID][chip->ChipID].Status = ChipStatus::CMD_IN;
			Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
				this, bookKeepingEntry, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 0, NOT_GC_WL);
			break;
		default:
