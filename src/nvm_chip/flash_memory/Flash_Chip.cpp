		...
		void Flash_Chip::start_command_execution(Flash_Command* command, sim_time_type suspendTime, bool is_GC_WL)
		{
			...
			if (command->Address.size() > 1
				...
				for (int i = 0; i < command->Address.size(); i++) {
					for (int j = 0; j < subblks.size(); j++) {
						if (command->Address[i].ChannelID == subblks[j].ChannelID && command->Address[i].ChipID == subblks[j].ChipID && command->Address[i].DieID == subblks[j].DieID && command->Address[i].PlaneID == subblks[j].PlaneID && command->Address[i].BlockID == subblks[j].BlockID) {
							if (command->CommandCode == CMD_ERASE || command->CommandCode == CMD_ERASE_BLOCK || command->CommandCode == CMD_ERASE_BLOCK_MULTIPLANE) {
...
#
			...
			if (ErsLoopBarrierTime > 0)  //when ERS_DEFER_SUS_ENABLE == true
			{
...
#else
				///* flag0213 doodu
#if REMOVE_GC_W_LAT_ALL
				if (command->CommandCode == CMD_PROGRAM_PAGE_MULTIPLANE || command->CommandCode == CMD_PROGRAM_PAGE ) {
					//std::cout << ".";
					targetDie->Expected_finish_time = Simulator->Time() + ErsLoopBarrierTime;
				}
				else {
					targetDie->Expected_finish_time = Simulator->Time() + ErsLoopBarrierTime + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
				}
				//*/
				//targetDie->Expected_finish_time = Simulator->Time() + ErsLoopBarrierTime + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
#elif REMOVE_LAT_GC_W
				if (is_GC_WL) {
					targetDie->Expected_finish_time = Simulator->Time();
					//targetDie->Expected_finish_time = Simulator->Time() + ErsLoopBarrierTime + (Get_command_execution_latency(command->CommandCode, command->Address[0].PageID))/ DEVISION_FACTOR; //flag0217
				}
				else {
					targetDie->Expected_finish_time = Simulator->Time() + ErsLoopBarrierTime + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
				}
			...
			else if (PgmISPPRemainingTime > 0)
			{
				...
				if (is_GC_WL) {
					targetDie->Expected_finish_time = Simulator->Time();
					//targetDie->Expected_finish_time = Simulator->Time() + PgmISPPRemainingTime + (Get_command_execution_latency(command->CommandCode, command->Address[0].PageID))/ DEVISION_FACTOR; //flag0217
				}
				else {
					targetDie->Expected_finish_time = Simulator->Time() + PgmISPPRemainingTime + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
				}
			...
			if (is_GC_WL) {
				targetDie->Expected_finish_time = Simulator->Time();
				//targetDie->Expected_finish_time = Simulator->Time() + suspendTime + (Get_command_execution_latency(command->CommandCode, command->Address[0].PageID))/ DEVISION_FACTOR; //flag0217
			}
			else {
				targetDie->Expected_finish_time = Simulator->Time() + suspendTime + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
			}
			...
			targetDie->CommandFinishEvent = Simulator->Register_sim_event(targetDie->Expected_finish_time,
				this, command, static_cast<int>(Chip_Sim_Event_Type::COMMAND_FINISHED), 0);
...
//			if (this->ChannelID == 3 && this->ChipID == 3)
			...
			targetDie->CommandFinishEvent = Simulator->Register_sim_event(targetDie->Expected_finish_time,
				this, targetDie->CurrentCMD, static_cast<int>(Chip_Sim_Event_Type::COMMAND_FINISHED, 0));
