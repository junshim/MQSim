...
//			gnStartPrint = true;
	...
	void IO_Flow_Synthetic::Start_simulation() 
	{
		...
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH)
			Simulator->Register_sim_event((sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0, 0);
		else
			Simulator->Register_sim_event((sim_time_type)1, this, 0, 0, 0);
	}
	...
	void IO_Flow_Synthetic::Execute_simulator_event(MQSimEngine::Sim_Event* event)
	{
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH)
		{
			...
			if (req != NULL)
			{
				...
				Simulator->Register_sim_event(Simulator->Time() + (sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0, 0);
			}
		}
		else
		{
