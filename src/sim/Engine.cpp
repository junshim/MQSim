...
namespace MQSimEngine
{
	...
	void Engine::Start_simulation()
	{
		...
		for(std::unordered_map<sim_object_id_type, Sim_Object*>::iterator obj = _ObjectList.begin(); obj != _ObjectList.end(); ++obj)
		{
...
*/
	...
	Sim_Event* Engine::Register_sim_event(sim_time_type fireTime, Sim_Object* targetObject, void* parameters, int type, int is_subblock, bool is_GCWL)
	{
		Sim_Event* ev = new Sim_Event(fireTime, targetObject, parameters, type);
#if REMOVE_LATENCY_SEND
		ev->is_subERS = is_subblock;
#endif
#if NEW_SUBMIT
		ev->is_GCWL = is_GCWL;
#endif
