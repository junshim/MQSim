...
extern FILE* free_log_file;
extern FILE* eviction_log_file;
extern FILE* eviction_interval_log_file;
extern FILE* free_log_file2;
extern FILE* pfile;
extern FILE* stream_id_log_file;
extern FILE* user_write_latency_log_file;
...
int main(int argc, char* argv[])
{
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++)
	{
		...
		sarsa_unit = new Sarsa(exec_params->SSD_Device_Configuration);
	...
	// close file
	fclose(free_log_file);
	fclose(eviction_log_file);
	fclose(eviction_interval_log_file);
	fclose(free_log_file2);
	fclose(pfile);
	fclose(stream_id_log_file);
	fclose(user_write_latency_log_file);
